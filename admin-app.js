...
document.getElementById("btnExportarReportePDF")?.addEventListener("click", async () => {
  ...
  const diasRows = Object.entries(diasMap).sort((a,b) => b[0].localeCompare(a[0])).map(([f, d]) => {
    ...
    return `<tr>
      <td>${parseInt(fd)}/${parseInt(fm)}/${fy}</td>
      <td style="text-align:right">${d.ventas}</td>
      <td style="text-align:right">${fmt(d.total)}</td>
      <td style="text-align:right">${fmt(Math.round(d.total / d.ventas))}</td>
    </tr>`;
  }).join("");

  const prodRows = topProds.map(([desc, d]) => `<tr>
    <td>${desc}</td>
  ...
  const content = `
    <div style="font-family:'DM Sans',sans-serif;font-size:13px;color:#111;padding:2rem;max-width:600px;margin:0 auto">
      <div style="font-size:18px;font-weight:600;margin-bottom:2px">JPSoft | QBV — Reporte</div>
      <div style="font-size:12px;color:#888;margin-bottom:1.5rem">${periodoLabel}</div>

      <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr 1fr;gap:10px;margin-bottom:1.5rem">
        ${[["Total",fmt(tot),"#111"],["Efectivo",fmt(totE),"#1a7a50"],["Mercado Pago",fmt(totM),"#009ee3"],["Débito",fmt(totD),"#185fa5"],["Crédito",fmt(totC),"#185fa5"]].map(([l,v,col]) => `
        <div style="border:1px solid #eee;border-radius:6px;padding:10px">
          <div style="font-size:10px;color:#aaa;text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px">${l}</div>
          <div style="font-size:15px;font-weight:600;color:${col}">${v}</div>
        </div>`).join("")}
      </div>

      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:1.5rem">
        <div>
          <div style="font-size:10px;font-weight:600;color:#aaa;text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px">Resumen por día</div>
          <table style="width:100%;border-collapse:collapse;font-size:11px">
            <thead><tr style="background:#f5f5f5">
              <th style="text-align:left;padding:5px 8px">Fecha</th>
              <th style="text-align:right;padding:5px 8px">Ventas</th>
              <th style="text-align:right;padding:5px 8px">Total</th>
              <th style="text-align:right;padding:5px 8px">Prom.</th>
            </tr></thead>
            <tbody>${diasRows}</tbody>
          </table>
        </div>
        <div>
          <div style="font-size:10px;font-weight:600;color:#aaa;text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px">Productos más vendidos</div>
          <table style="width:100%;border-collapse:collapse;font-size:11px">
            <thead><tr style="background:#f5f5f5">
              <th style="text-align:left;padding:5px 8px">Producto</th>
              <th style="text-align:right;padding:5px 8px">Unid.</th>
              <th style="text-align:right;padding:5px 8px">Total</th>
            </tr></thead>
      ...
      <div style="font-size:11px;color:#bbb;text-align:center;margin-top:1rem">Generado el ${now} · JPSoft | QBV</div>
    </div>`;
  ...
  try {
    ...
    pdf.save(`JPSoft_QBV_Reporte_${desde}_${hasta}.pdf`);
