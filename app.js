...
function renderModalVenta() {
  ...
  if (ventaCart) {
    ...
    keys.forEach(k => {
      ...
      row.style.cssText = "display:flex;align-items:center;padding:8px 12px;border-bottom:1px solid var(--border);gap:8px;font-size:13px";
      row.innerHTML = `
      ...
      ventaCart.appendChild(row);
    });
    // Quitar border del último
    if (ventaCart.lastChild) ventaCart.lastChild.style.borderBottom = "none";

    // Delegación de eventos en modal
    ventaCart.addEventListener("click", e => {
      const btn = e.target.closest("[data-action]");
      if (!btn) return;
      const action = btn.dataset.action;
      const key    = btn.dataset.key;
  ...
  const dRow = document.getElementById("cartDescuentoRow");
  const dLabel = document.getElementById("cartDescuentoLabel");
  const dMonto = document.getElementById("cartDescuentoMonto");
  const dBar   = document.getElementById("cobrarDescMonto");

  if (sEl) sEl.textContent = fmtDec(subtotal);
  if (tEl) tEl.textContent = fmtDec(total);
  if (dBar) dBar.textContent = descMonto > 0 ? "− " + fmtDec(descMonto) : "";

  if (dRow) {
    if (descMonto > 0) {
      dRow.style.display = "flex";
      const sufijo = descuentoTipo === "pct" ? ` (${descuentoValor}%)` : "";
      if (dLabel) dLabel.textContent = "Descuento" + sufijo;
      if (dMonto) dMonto.textContent = "− " + fmtDec(descMonto);
    } else {
      dRow.style.display = "none";
    }
  }

  // También actualizar barra cobrar
  const cobrarTotal = document.getElementById("cobrarTotal");
  const cobrarItems = document.getElementById("cobrarItems");
  if (cobrarTotal) cobrarTotal.textContent = fmtDec(total);
  if (cobrarItems) cobrarItems.textContent = keys.length + (keys.length === 1 ? " ítem" : " ítems");
