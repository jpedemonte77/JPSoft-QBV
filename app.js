...
let provColorMap  = {};
let cajaHoy       = null; // { apertura: {hora,fondo,turno}, ventas: {}, cierre: {} }
...
function initFirebase() {
  ...
  // Caja del día
  onValue(ref(db, `caja/${todayKey()}`), snap => {
    cajaHoy = snap.val() || null;
    renderCaja();
    updateCajaSidebar();
  });
}
...
function renderCaja() {
  ...
  document.getElementById("cajaTituloFecha").textContent =
    "Caja — " + new Date().toLocaleDateString("es-AR", { weekday:"long", day:"2-digit", month:"long", year:"numeric" });

  const apertura = cajaHoy?.apertura;
  const cierre   = cajaHoy?.cierre;
  const ventas   = cajaHoy?.ventas ? Object.values(cajaHoy.ventas) : [];

  if (!apertura) {
    // Caja cerrada: mostrar formulario apertura
    document.getElementById("cajaAperturaWrap").classList.remove("hidden");
    document.getElementById("cajaAbiertaWrap").classList.add("hidden");
    document.getElementById("btnCerrarCaja").classList.add("hidden");
    document.getElementById("cajaEstadoBadge").innerHTML = `<span class="badge badge-neutral">Cerrada</span>`;
    document.getElementById("aperturaFecha").textContent =
      new Date().toLocaleDateString("es-AR", { day:"2-digit", month:"2-digit", year:"numeric" });
    document.getElementById("cajaSubtitulo").textContent = "La caja no fue abierta hoy";
    return;
  }
  ...
  document.getElementById("btnCerrarCaja").classList.toggle("hidden", yaHayCierre);
  ...
  if (!ventas.length) {
    timeline.innerHTML = `<div class="empty-row">No hay ventas registradas hoy.</div>`;
  } else {
  ...
  // Resumen cierre
  if (yaHayCierre) {
    document.getElementById("cierreSummaryWrap").classList.remove("hidden");
...
function updateCajaSidebar() {
  ...
  if (cajaHoy?.apertura && !cajaHoy?.cierre) {
    state.textContent = "Abierta";
    state.className = "caja-sidebar-state caja-open";
  } else {
    state.textContent = cajaHoy?.cierre ? "Cerrada" : "Sin abrir";
...
document.getElementById("btnAbrirCaja").addEventListener("click", async () => {
  ...
  await set(ref(db, `caja/${todayKey()}/apertura`), { hora, fondo, turno, admin });
...
document.getElementById("btnConfirmarCierre").addEventListener("click", async () => {
  await set(ref(db, `caja/${todayKey()}/cierre`), {
    hora:  nowHora(),
    admin: getNombreUsuario()
  });
